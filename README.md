# Steeltoe tap-steeltoe-web-app Accelerator
...
```
tanzu apps workload create tap-steeltoe-web-app -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/tap-steeltoe-web-app-source \
  --type web
```
...
```
tanzu apps workload get tap-steeltoe-web-app
```
...
```
dotnet publish -o publish tap-steeltoe-web-app.csproj
```

Push the app to Cloud Foundry:

```
cf push tap-steeltoe-web-app -p ./publish --random-route
```
